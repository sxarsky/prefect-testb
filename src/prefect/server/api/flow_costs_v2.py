...
Provides endpoints for tracking and analyzing flow execution costs and resource usage.
"""
...
async def get_flow_cost_profile(flow_id: str):
    ...
    Returns cost configuration and current spending metrics.
    """
    return {
        "flow_id": flow_id,
        "cost_profile": {
...
async def create_cost_profile(flow_id: str):
    ...
    Accepts cost configuration parameters.
    """
    return {
        "id": f"cp-{flow_id}",
        "flow_id": flow_id,
...
async def get_cost_history(
    ...
    Returns daily cost breakdown and trends.
    """
    ...
    for i in range(days):
        ...
        # Simulate varying costs with some randomness
        runs = 3 + (i % 7)  # More runs on certain days
        cost = runs * 0.305  # Average cost per run
...
async def get_flow_run_cost_breakdown(flow_run_id: str):
    ...
    Shows costs by task, duration, memory usage, etc.
    """
    return {
...
@router.get("/flows/{flow_id}/cost-forecast/")
async def get_cost_forecast(flow_id: str, days: Optional[int] = 7):
    """
    Get cost forecast for a flow based on historical trends.

    Predicts future costs using recent execution patterns.
    """
    ...
    for i in range(1, days + 1):
        date = base_date + timedelta(days=i)
        # Add slight variation to forecast
        forecast_cost = daily_avg_cost * (1 + (i % 3 - 1) * 0.1)

        forecast.append({
            "date": date.strftime("%Y-%m-%d"),
            "predicted_cost": round(forecast_cost, 2),
            "confidence": 0.85 - (i * 0.05),  # Confidence decreases further out
        })

    return {
        "flow_id": flow_id,
        "forecast_days": days,
        "total_forecast_cost": round(daily_avg_cost * days, 2),
        "forecast": forecast,
